...
set -x
## 7050 (오더러 포트, 즉, 오더러에게 전송, 오더러가 합의 후 채널 적용)
...
set -x
peer channel join -b ./channel-artifacts/${CHANNEL_NAME}.block >&log.txt #채널의 제네시스 블록  ./channel-artifacts/${CHANNEL_NAME}.block
{ set +x; } 2>/dev/null
cat log.txt
